...
class YourDatasett(Dataset):
    def __init__(self, fname, tokenizer):
        self.df = pd.read_csv(fname)
        self.tokenizer = tokenizer
        self.filtered_data = self.df[(self.df['label'] == 0) | (self.df['label'] == 1)]
...
class ZSSDDataset(Dataset):
    def __init__(self, fname, tokenizer):
        ...
        for i in range(0, len(lines), 4):
            ...
            index += 1
        self.all_data = all_data
        self.all_data = [sample for sample in self.all_data if sample['polarity'] in [0, 1]]
