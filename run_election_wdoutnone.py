...
from utils.data_utils import  Tokenizer4Bert, ZSSDDataset,YourDatasett
...
class Instructor(object):
    def __init__(self,opt):
        ...
        bert_proto= BertModel.from_pretrained('bert-base-uncased')
        ...
        self.trainset = YourDatasett(fname=self.opt.train_dir,tokenizer=tokenizer)
        self.valset = YourDatasett(fname=self.opt.dev_dir, tokenizer=tokenizer)
        self.testset = YourDatasett(fname=self.opt.test_dir,tokenizer=tokenizer)
        self.label_mapping = {
            "FAVOR": 0,
            "NONE": 1,
            "AGAINST": 2
    ...
    def train_traditon(self):
        ...
        for i_epoch in range(self.opt.epochs):
            ...
            for i_batch, batch in enumerate(train_loader):
                ...
                #true_stance = [self.label_mapping[label] for label in true_stance]
                if opt.n_gpus > 0:
    ...
    def dev_tradition(self):
        ...
        f1_ma = f1_score(all_labels, preds, labels=[0,2], average='micro')
    ...
    def test_tradition(self):
        ...
        report = classification_report(all_labels, preds, target_names=['Con','Pro'], digits=4,zero_division=1)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', default='biden', type=str,required=False)
    parser.add_argument('--output_par_dir',default='test_outputs',type=str)
    parser.add_argument('--polarities', default='tp', nargs='+', help="if just two polarity switch to ['positive', 'negtive']",required=False)
    ...
    dataset_files = {
        'trump': {
            'train': './datasets/election_2020/augmented_data_trump_p_train.csv',
            'test': './datasets/election_2020/augmented_data_trump_p_t.csv',
        },
         'biden': {
            'train': './datasets/election_2020/augmented_data_with_p.csv',
            'test': './datasets/election_2020/augmented_data_with_p_t.csv',
        },
    }
    polarities = {
        'sem16_naacl': [0, 2],   # 0 for Against, 1 for Favor, 2 for none
